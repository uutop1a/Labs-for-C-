...
1) Получить в другом файле последовательного доступа все компоненты исходного файла, кроме тех, которые кратны k.
2) Скопировать элементы заданного файла в квадратную матрицу размером n×n (если элементов файла недостает, заполнить оставшиеся элементы матрицы нулями). Указать столбец (назвать его номер), где произведение элементов дальше всего от нуля
3) Файл содержит сведения об игрушках: название игрушки, ее стоимость в рублях и возрастные границы (например, игрушка может предназначаться для детей от двух до пяти лет). Получить сведения о том, можно ли подобрать игрушку, любую, кроме мяча, подходящую ребенку трех лет.
4) В файле найти сумму максимального и минимального элементов.
5) Вычислить сумму чётных элементов.
6) Создать новый текстовый файл, каждая строка которого содержит первый символ соответствующей строки исходного файла.
   
Задание 1
Создаётся бинарный файл с целыми числами.
Затем в другой бинарный файл копируются все числа, кроме тех, которые кратны k.

Задание 2
Числа из бинарного файла копируются в квадратную матрицу n × n.
Если чисел не хватает — недостающие места заполняются нулями.
После этого находится номер столбца, у которого произведение элементов дальше всего от нуля (максимально по модулю).

Задание 3
Работа с бинарным файлом структур Toy:
название,
цена,
возрастной диапазон.
Проверяется: можно ли подобрать игрушку для ребёнка 3 лет, кроме мяча.

Задание 4
Текстовый файл: по одному целому числу в строке.
Находится минимум, максимум и их сумма.

Задание 5
Текстовый файл: в строках по несколько чисел.
Считается сумма всех чётных элементов.

Задание 6
Создаётся новый текстовый файл, где каждая строка — это первый символ соответствующей строки исходного файла.

Тест:
Ex 1
Source file: 2 -12 -9 -12 19 14 -11 19 11 9 6 10 17 13 1 17 -19 5 18 3 3 -16 13 17 -19 18 -2 5 0 20
Without multiples 3: 2 19 14 -11 19 11 10 17 13 1 17 -19 5 -16 13 17 -19 -2 5 20

 Ex 2
Numbers (file): -3 -3 3 2 -1 4 2 -4 4 -1 5 -5 -4 -2 0 -1 -3 -3 0 -4
Matrix 4x4:
    -3     -3      3      2
    -1      4      2     -4
     4     -1      5     -5
    -4     -2      0     -1
Column with the product farthest from zero: 1 (numbering ╤Б 1), |prod|=48

 Ex 3
Contents of the toy file:
Name: ball, cost: 300, age: 3-10
Name: yoyo, cost: 1200, age: 3-7
Name: monopoly, cost: 500, age: 2-5
Name: lego, cost: 2500, age: 5-12
Name: car, cost: 800, age: 3-6
Found a suitable toy for a 3-year-old (not a ball)): yoyo, cost: 1200, age: 3-7

 Ex 4
min=-97, max=89, sum(min+max)=-8

 Ex 5
Sum of even elements = -124

 Ex 6
file created task6_firstchars.txt (the first characters of each line).
